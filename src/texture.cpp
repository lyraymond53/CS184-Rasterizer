...
Color Texture::sample(const SampleParams &sp) {
  ...
  // Should return a color sampled based on the psm and lsm parameters given
    ...
    int starting_index = 3*width*sp.p_uv[1] + 3*sp.p_uv[0];

    float r = this->mipmap[get_level(sp)].texels[starting_index]/255;
    float g = this->mipmap[get_level(sp)].texels[starting_index + 1]/255;
    float b = this->mipmap[get_level(sp)].texels[starting_index + 2]/255;
...
Color Texture::sample_nearest(Vector2D uv, int level) {
  ...
  Vector2D nearest_pixel_index = Vector2D(floor(uv[0]), floor(uv[1]));
  return Color();
}
