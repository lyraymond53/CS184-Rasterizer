...
private:
  ...
  struct SampleBuffer {
    ...
    Color get_pixel_color() {
        ...
        Color c = Color(0, 0, 0);
        int num_samples = samples_per_side * samples_per_side;
      for (int i = 0; i < samples_per_side; i++) {
          for (int j = 0; j < samples_per_side; j++) {

              c[0] += pow(sub_pixels[i][j][0], 2)/(num_samples);
              c[1] += pow(sub_pixels[i][j][1], 2)/(num_samples);
              c[2] += pow(sub_pixels[i][j][2], 2)/(num_samples);
          }
      }
      return Color(sqrt(c[0])/255, sqrt(c[1])/255, sqrt(c[2])/255);
