...
// Rasterize a triangle.
    void DrawRend::rasterize_triangle(float x0, float y0,
        ...
        float minx = min(min(x0, min(x1, x2)), 0.f);
        float maxx = max(max(x0, max(x1, x2)), (float) width);

        float miny = min(min(y0, min(y1, y2)), 0.f);
        float maxy = max(max(y0, max(y1, y2)), (float) height);
        ...
        float increment = sqrt(sample_rate) / 2;

        Vector3D empty3D = Vector3D(0, 0, 0);

        for (int i = minx; i < maxx; i++) {
            for (int j = miny; j < maxy; j++) {

                for (int incx = 0; incx < sqrt(sample_rate); incx++) {

                    float L1a = -(i + incx + increment - x0) * y1y0;
                    float L2a = -(i + incx + increment - x1) * y2y1;
                    float L3a = -(i + incx + increment - x2) * y0y2;

                    for (int incy = 0; incy < sqrt(sample_rate); incy++) {

                        float L1 = L1a + (j + incy + increment - y0) * x1x0;
                        float L2 = L2a + (j + incy + increment - y1) * x2x1;
                        float L3 = L3a + (j + incy + increment - y2) * x0x2;

                        SampleParams sp = {};
                        sp.lsm = this->lsm;
                        sp.psm = this->psm;
                        sp.p_dx_uv = Vector2D(0, 0);
                        sp.p_dy_uv = Vector2D(0, 0);
                        // Still missing p_uv coordinates in SampleParams

                        if ((L1 >= 0 && L2 >= 0 && L3 >= 0)) {
                            if (tri != nullptr) {
                                //Compute barycentric coordinates for color
                                float alpha = (-(i + incx - x1) * y2y1 + (j + incy - y1) * x2x1) /
                                              (x1x0 * y2y1 + (-y1y0 * x2x1));
                                float beta = (-(i + incx - x2) * y0y2 + (j + incy - y2) * x0x2) /
                                             (x2x1 * y0y2 + (-y2y1 * x0x2));
                                float gamma = 1 - alpha - beta;

                                Vector3D p_bary = Vector3D(alpha, beta, gamma);

                                color = tri->color(p_bary, empty3D, empty3D, sp);

                                samplebuffer[j][i].fill_color(incx, incy, color);
                            }
                            samplebuffer[j][i].fill_color(incx, incy, color);
